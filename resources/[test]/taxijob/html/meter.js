...
const updateMeter = (meterData) => {
    if (typeof meterData.fareAmount !== "number") console.error("fareAmount is not a number");
    if (typeof meterData.distanceTravelled !== "number") console.error("distanceTravelled is not a number");

    $("#total-price").html("£ " + meterData.fareAmount.toFixed(2));
    $("#total-distance").html(meterData.distanceTravelled.toFixed(2) + " mi");
};
const resetMeter = () => {
...
const meterToggle = () => {
    if (!meterStarted) {
        $.post(
            "https://stag_taxijob/toggleMeter",
            JSON.stringify({
                enabled: true,
            })
        );
