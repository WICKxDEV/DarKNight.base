...
$(document).on("click", ".bank-app-header-button", function (e) {
  ...
  if (CurrentTab != PressedTab) {
    var PreviousObject = $(".bank-app-header").find(
      '[data-headertype="' + CurrentTab + '"]'
    );
...
QB.Phone.Functions.DoBankOpen = function () {
  QB.Phone.Data.PlayerData.money.bank =
    QB.Phone.Data.PlayerData.money.bank.toFixed();
  $(".bank-app-account-number").val(QB.Phone.Data.PlayerData.charinfo.account);
  $(".bank-app-account-balance").html(
    "LKR " + QB.Phone.Data.PlayerData.money.bank
  );
  $(".bank-app-account-balance").data(
    "balance",
    QB.Phone.Data.PlayerData.money.bank
  );
...
$(document).on("click", "#accept-transfer", function (e) {
  ...
  if (iban != "" && amount != "") {
    $.post(
      "https://qb-phone/CanTransferMoney",
      JSON.stringify({
        sendTo: iban,
        amountOf: amount,
      }),
      function (data) {
        if (data.TransferedMoney) {
          ...
          $(".bank-app-account-balance").html(
            "LKR " + data.NewBalance.toFixed(0)
          );
          $(".bank-app-account-balance").data(
            "balance",
            data.NewBalance.toFixed(0)
          );
          QB.Phone.Notifications.Add(
            "fas fa-university",
            "QBank",
            "You have transfered LKR " + amount + "!",
            "#badc58",
            1500
          );
        } else {
      ...
      function (CanPay) {
        if (CanPay) {
          ...
          QB.Phone.Notifications.Add(
            "fas fa-university",
            "QBank",
            "You have paid LKR " + InvoiceData.amount + "!",
            "#badc58",
            1500
          );
          var amountData = $(".bank-app-account-balance").data("balance");
...
QB.Phone.Functions.LoadBankInvoices = function (invoices) {
  if (invoices !== null) {
    ...
    $.each(invoices, function (i, invoice) {
      var Elem =
        '<div class="bank-app-invoice" id="invoiceid-' +
        i +
        '"> <div class="bank-app-invoice-title">' +
        invoice.society +
        ' <span style="font-size: 1vh; color: gray;">(Sender: ' +
        invoice.sender +
        ')</span></div> <div class="bank-app-invoice-amount">LKR ' +
        invoice.amount +
        '</div> <div class="bank-app-invoice-buttons"> <i class="fas fa-check-circle pay-invoice"></i> <i class="fas fa-times-circle decline-invoice"></i> </div> </div>';
...
QB.Phone.Functions.LoadContactsWithNumber = function (myContacts) {
  ...
  if (myContacts !== null) {
    $.each(myContacts, function (i, contact) {
      ...
      var ContactElement =
        '<div class="bank-app-my-contact" data-bankcontactid="' +
        i +
        '"> <div class="bank-app-my-contact-firstletter">' +
        contact.name.charAt(0).toUpperCase() +
        '</div> <div class="bank-app-my-contact-name">' +
        contact.name +
        "</div> </div>";
...
$(document).on("click", ".bank-app-my-contact", function (e) {
  ...
  if (
    PressedContactData.iban !== "" &&
    PressedContactData.iban !== undefined &&
    PressedContactData.iban !== null
  ) {
    ...
    QB.Phone.Notifications.Add(
      "fas fa-university",
      "QBank",
      "There is no bank account attached to this number!",
      "#badc58",
      2500
    );
  }
