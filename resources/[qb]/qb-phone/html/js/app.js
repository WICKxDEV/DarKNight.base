...
QB.Phone.Functions.SetupApplications = function (data) {
  ...
  for (i = 1; i <= QB.Phone.Data.MaxSlots; i++) {
    var applicationSlot = $(".phone-applications").find(
      '[data-appslot="' + i + '"]'
    );
...
QB.Phone.Functions.SetupAppWarnings = function (AppData) {
  $.each(AppData, function (i, app) {
    var AppObject = $(".phone-applications")
      .find("[data-appslot='" + app.slot + "']")
      .find(".app-unread-alerts");
  ...
  if (AppObject.length !== 0) {
    if (CanOpenApp) {
      if (QB.Phone.Data.currentApplication == null) {
        QB.Phone.Animations.TopSlideDown(
        ...
        if (PressedApplication == "settings") {
          ...
          $("#mySerialNumber").text(
            "QB-" + QB.Phone.Data.PlayerData.metadata["phonedata"].SerialNumber
          );
        } else if (PressedApplication == "twitter") {
          $.post(
            "https://qb-phone/GetMentionedTweets",
            JSON.stringify({}),
            function (MentionedTweets) {
              QB.Phone.Notifications.LoadMentionedTweets(MentionedTweets);
            }
          );
          $.post(
            "https://qb-phone/GetHashtags",
            JSON.stringify({}),
            function (Hashtags) {
          ...
          $.post(
            "https://qb-phone/GetBankContacts",
            JSON.stringify({}),
            function (contacts) {
          ...
          $.post(
            "https://qb-phone/GetWhatsappChats",
            JSON.stringify({}),
            function (chats) {
          ...
          $.post(
            "https://qb-phone/GetSuggestedContacts",
            JSON.stringify({}),
            function (suggested) {
          ...
          $.post(
            "https://qb-phone/ClearGeneralAlerts",
            JSON.stringify({
              app: "phone",
            })
          );
          ...
          $.post(
            "https://qb-phone/SetupGarageVehicles",
            JSON.stringify({}),
            function (Vehicles) {
          ...
          $.post(
            "https://qb-phone/GetCryptoTransactions",
            JSON.stringify({}),
            function (data) {
          ...
          $.post(
            "https://qb-phone/GetAvailableRaces",
            JSON.stringify({}),
            function (Races) {
              SetupRaces(Races);
            }
          ...
          $.post(
            "https://qb-phone/GetPlayerHouses",
            JSON.stringify({}),
            function (Houses) {
            ...
            function (Keys) {
              ...
              if (Keys.length > 0) {
                $.each(Keys, function (i, key) {
                  var elem =
                    '<div class="mykeys-key" id="keyid-' +
                    i +
                    '"><span class="mykeys-key-label">' +
                    key.HouseData.adress +
                    '</span> <span class="mykeys-key-sub">Click to set GPS</span> </div>';
          ...
          $.post(
            "https://qb-phone/GetCurrentLawyers",
            JSON.stringify({}),
            function (data) {
              SetupLawyers(data);
            }
    ...
    if (PressedApplication != null) {
      QB.Phone.Notifications.Add(
        ...
        "System",
        QB.Phone.Data.Applications[PressedApplication].tooltipText +
          " is not available!"
      );
    }
  }
...
$(document).on("click", ".mykeys-key", function (e) {
  ...
  $.post(
    "https://qb-phone/SetHouseLocation",
    JSON.stringify({
      HouseData: KeyData,
    })
  );
    ...
    if (QB.Phone.Data.currentApplication == "whatsapp") {
      ...
      if (CurrentTab == "invoices") {
        setTimeout(function () {
          ...
          var InvoicesObjectBank = $(".bank-app-header").find(
            '[data-headertype="invoices"]'
          );
          var HomeObjectBank = $(".bank-app-header").find(
            '[data-headertype="accounts"]'
          );
...
QB.Phone.Notifications.Add = function (icon, title, text, color, timeout) {
  $.post("https://qb-phone/HasPhone", JSON.stringify({}), function (HasPhone) {
    if (HasPhone) {
      ...
      if (
        QB.Phone.Notifications.Timeout == undefined ||
        ...
        if (icon !== "politie") {
          ...
          $(".notification-icon").html(
            '<img src="./img/politie.png" class="police-icon-notify">'
          );
        }
...
QB.Phone.Functions.UpdateTime = function (data) {
  ...
  $("#phone-time").html(
    "<span>" + data.InGameTime.hour + ":" + data.InGameTime.minute + "</span>"
  );
};
...
QB.Screen.popUp = function (source) {
  if (!up) {
    ...
    $("<img  src=" + source + ' style = "width:100%; height: 100%;">').appendTo(
      ".popupclass"
    );
    up = true;
  }
    ...
    switch (event.data.action) {
      ...
      case "Notification":
        QB.Screen.Notification(
          event.data.NotifyData.title,
          event.data.NotifyData.content,
          event.data.NotifyData.icon,
          event.data.NotifyData.timeout,
          event.data.NotifyData.color
        );
        break;
      case "PhoneNotification":
        QB.Phone.Notifications.Add(
          event.data.PhoneNotify.icon,
          event.data.PhoneNotify.title,
          event.data.PhoneNotify.text,
          event.data.PhoneNotify.color,
          event.data.PhoneNotify.timeout
        );
        break;
      ...
      case "UpdateMentionedTweets":
        ...
        break;
      case "UpdateBank":
        $(".bank-app-account-balance").html("LKR " + event.data.NewBalance);
      ...
      case "UpdateChat":
        if (QB.Phone.Data.currentApplication == "whatsapp") {
          if (
            OpenedChatData.number !== null &&
            OpenedChatData.number == event.data.chatNumber
          ) {
      ...
      case "UpdateCallTime":
        ...
        if (!QB.Phone.Data.IsOpen) {
          ...
          $(".call-notifications-title").html(
            "In conversation (" + timeString + ")"
          );
          $(".call-notifications-content").html(
            "Calling with " + event.data.Name
          );
