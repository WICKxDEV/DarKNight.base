...
$("document").ready(function () {
  window.addEventListener("message", function (event) {
    ...
    if (item.type == "setupContract") {
      ...
      $("#property-price").html("LKR " + item.houseprice);
      $("#property-brokerfee").html("LKR " + item.brokerfee);
      $("#property-bankfee").html("LKR " + item.bankfee);
      $("#property-taxes").html("LKR " + item.taxes);
      $("#property-totalprice").html("LKR " + item.totalprice);
    }
    ...
    if (item.type == "buyOption") {
      ...
      $(".decorate-confirm")
        .find("p")
        .html(
          "Are you sure you want to purchase the item for LKR " +
            selectedObjectData.price +
            "?"
        );
    }
  ...
  document.onkeyup = function (data) {
    ...
    if (data.which == 13) {
      if (
        ...
        $.post(
          "https://qb-houses/editOwnedObject",
          JSON.stringify({
        ...
        selectedObjectData = null;
        $.post(
          "https://qb-houses/setupMyObjects",
          JSON.stringify({}),
          function (myObjects) {
            $(".decorate-items").html("");
            $.each(myObjects, function (i, object) {
              var elem =
                '<div class="decorate-item" id="myobject-' +
                i +
                '" data-type="myObject"><span id="decorate-item-name"><b>Object: </b>' +
                object.hashname +
                '</span><span id="decorate-item-category"><strong>Price: </strong><span id="item-price" style="color: green;">OWNED</span></span></div>';
...
$(document).on("click", ".footer-btn", function () {
  ...
  if (selectedCategory != "remove-owned-obj") {
    ...
    $.each(houseCategorys[selectedCategory].items, function (i, item) {
      var elem =
        '<div class="decorate-item" id="object-' +
        i +
        '" data-type="newObject"><span id="decorate-item-name"><b>Object: </b>' +
        item.label.charAt(0).toUpperCase() +
        "" +
        item.label.substr(1).toLowerCase() +
        '</span><span id="decorate-item-category"><strong>Price: </strong><span id="item-price" style="color: green;">LKR ' +
        item.price +
        "</span></span></div>";
      $(".decorate-items").append(elem);
...
$(document).on("click", "#buy-object", function () {
  $.post(
    "https://qb-houses/buySelectedObject",
    JSON.stringify({
...
$(document).on("click", ".decorate-item", function () {
  ...
  if ($("#" + objId).data("type") == "newObject") {
    ...
    if (selectedObject == this) {
      ...
      $.post(
        "https://qb-houses/selectOwnedObject",
        JSON.stringify({
      ...
      $(".decorate-footer-buttons").html("");
      var elem =
        '<div class="footer-btn" id="remove-owned-obj"><p>Remove</p></div>';
      $(".decorate-footer-buttons").append(elem);
      $(".decorate-footer-buttons").fadeIn(150);
    }
  }
